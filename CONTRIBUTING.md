...
- Take a look at [existing issues](https://github.com/remysaissy/aide-autodoc/issues).
- If you need to create an issue:
    - Make sure to clearly describe it.
    - Including steps to reproduce when it is a bug.
    - Include the version of aide-autodoc used.
...
Examples:
```
feat(macro): support nested doc comments
fix: handle empty doc comment gracefully
docs: update README with usage examples
chore(deps): bump aide to 0.16
```
...
We try to mark issues with a suggested level of experience (in Rust).
Where possible we try to spell out how to go about implementing the feature.

To start with, check out:
- Issues labeled as ["good first issue"](https://github.com/remysaissy/aide-autodoc/issues?q=is%3Aopen+is%3Aissue+label%3A%22good+first+issue%22).
- Issues labeled as ["Easy"](https://github.com/remysaissy/aide-autodoc/issues?q=is%3Aopen+is%3Aissue+label%3AE-easy).
...
git clone https://github.com/remysaissy/aide-autodoc.git
cd aide-autodoc
...
cargo run --example basic_usage -p aide-autodoc
...
Tests are in `aide-autodoc/tests/`. Integration tests are in `aide-autodoc/tests/integration.rs`; compile-pass tests use trybuild in `aide-autodoc/tests/trybuild/`. When adding features, add corresponding tests following the existing patterns.

See [Running Tests](#running-tests) for all test commands and coverage details.

### Inspecting Generated Code

To see what code aide-autodoc generates, use `cargo-expand`:
...
- Discuss something directly in the [Github issue](https://github.com/remysaissy/aide-autodoc/issues).
...
cargo test test_summary_extracted_from_first_paragraph         # Run a specific test
...
7. Publishes `aide-autodoc` to crates.io
